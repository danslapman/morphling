...
object FromJson {
    ...
    new FromJson[AnnotatedSchema[P, A, _]] {
      override val decoder: AnnotatedSchema[P, A, _] ~> Decoder = new (AnnotatedSchema[P, A, _] ~> Decoder) {
        override def apply[I](schema: AnnotatedSchema[P, A, I]): Decoder[I] = {
          HFix.cataNT[[Y1[_], Z1] =>> HEnvT[A, [Y[_], Z] =>> SchemaF[P, Y, Z], Y1, Z1], Decoder](annDecoderAlg[P, A]).apply(schema)
        }
      }
    }
  ...
  def annDecoderAlg[P[_] : FromJson, Ann[_]](implicit interpret: Ann ~> ([T] =>> Endo[Decoder[T]])): HAlgebra[[Y1[_], Z1] =>> HEnvT[Ann, [Y[_], Z] =>> SchemaF[P, Y, Z], Y1, Z1], Decoder] =

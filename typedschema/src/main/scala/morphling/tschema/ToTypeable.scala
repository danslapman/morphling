...
object ToTypeable {
  ...
  implicit def annSchemaToTypeable[P[_]: ToTypeable, A[_]: *[_] ~> λ[T => Endo[SwaggerTypeable[T]]]]
      : ToTypeable[AnnotatedSchema[P, A, *]] =
    new ToTypeable[AnnotatedSchema[P, A, *]] {
      override val toTypeable: AnnotatedSchema[P, A, *] ~> SwaggerTypeable =
        new (AnnotatedSchema[P, A, *] ~> SwaggerTypeable) {
          ...
          SwaggerTypeable.make(
            SwaggerOneOf(
              s.discriminator.cata(
                      ...
                      Option(id) -> Eval.now(b.typ match {
                        case SwaggerObject(properties, required, discriminator) =>
                          SwaggerRef(
                            ...
                            Eval.now(
                              SwaggerObject(
                                properties :+ discriminatorProp(id),
                                required.map(_ :+ dField),
                                discriminator
                              )
                            )
                          )
                ...
                s.alts
                  .map { case Alt(field, b, p) =>
                    Option.empty[String] -> Eval.now(
                      SwaggerObject(
                        Vector(
  ...
  def annTypAlg[P[_]: ToTypeable, Ann[_]](implicit
      interpret: Ann ~> λ[T => Endo[SwaggerTypeable[T]]]
  ): HAlgebra[HEnvT[Ann, SchemaF[P, *[_], *], *[_], *], SwaggerTypeable] =
  ...
  def recordTypeable[P[_]: ToTypeable, I](
      rb: FreeApplicative[PropSchema[I, SwaggerTypeable, *], I]
  ): SwaggerTypeable[I] = {

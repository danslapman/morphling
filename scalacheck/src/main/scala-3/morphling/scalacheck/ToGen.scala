...
trait ToGen[S[_]] extends Serializable {
  ...
  extension[S[_], A](s: S[A])(using TG: ToGen[S])
    def gen: Gen[A] = TG.toGen(s)
}
...
object ToGen {
  ...
  def annGenAlg[P[_] : ToGen, Ann[_]](using interpret: Ann ~> ([T] =>> Endo[Gen[T]])): HAlgebra[[Y1[_], Z1] =>> HEnvT[Ann, [Y[_], Z] =>> SchemaF[P, Y, Z], Y1, Z1], Gen] =
